...
void ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    {
        printf("nginx::ngx_process_events_and_timers::ngx_timer_resolution\n");
        timer = NGX_TIMER_INFINITE;
        flags = 0;
    }
    else
    {
        printf("nginx::ngx_process_events_and_timers::(no)ngx_timer_resolution\n");
    ...
    printf("nginx::ngx_process_events->init\n");
    (void)ngx_process_events(cycle, timer, flags);

    printf("nginx::ngx_process_events->end\n");
    delta = ngx_current_msec - delta;

    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "timer delta: %M", delta);

    printf("nginx::ngx_event_process_posted\n");
...
{
    ngx_connection_t *c;
    printf("nginx::ngx_handle_write_event\n");
